...
def normalize_to_continuum(wavelengths, flux, window_size=50, poly_order=3, sigma=3.0):
    ...
    """
    # ── Adaptar window_size a la resolución del espectro ──────────────────────
    # Para espectros de alta resolución (p. ej. 0.02 Å/px) el window_size=50
    # cubriría solo ~1 Å — menos que una línea espectral — y el continuo falla.
    # Objetivo: cada ventana debe cubrir al menos 30 Å del eje λ.
    if len(wavelengths) > 1:
        pixel_scale = (float(wavelengths[-1]) - float(wavelengths[0])) / len(wavelengths)
        if pixel_scale > 0:
            target_angstrom = 30.0
            adaptive_window = int(target_angstrom / pixel_scale)
            window_size = max(window_size, adaptive_window)
    # Nunca más del 5% del espectro total (evitar una sola ventana gigante)
    window_size = min(window_size, max(50, len(flux) // 20))
...
# ============================================================================
# ESTIMACIÓN DE FWHM DE LÍNEAS DE BALMER (para estimación de log g)
# ============================================================================

def estimate_balmer_fwhm(wavelengths, flux_normalized, line_center, window_width=40.0):
    """
    Estima el FWHM (Å) de una línea de Balmer asumiendo continuo normalizado = 1.0.

    A diferencia de ``estimate_fwhm`` (pensada para líneas metálicas estrechas),
    esta función:

    * Usa **continuo = 1.0** en lugar de percentil local, ya que las alas de
      Stark de las líneas de Balmer llenan casi toda la ventana de búsqueda.
    * Usa una ventana más amplia (defecto 40 Å) para capturar las alas ensanchadas
      de estrellas tipo A con alta gravedad (FWHM hasta ~30–35 Å en enanas A0).

    El FWHM de Balmer es un indicador directo de presión electrónica y, por tanto,
    de la gravedad superficial log(g): a mayor log(g), mayor presión, más Stark,
    mayor FWHM. Calibración: ``estimate_logg`` en ``luminosity_classification.py``.
    ...
    flux_normalized : array
        Flujo normalizado al continuo (continuo ≈ 1.0)
    line_center : float
        Longitud de onda central de la línea de Balmer en Å
    window_width : float
        Ventana de búsqueda total en Å (default: 40 Å → ±20 Å)
    ...
    float
        FWHM estimado en Å. Devuelve 0.0 si no puede estimarse.
    """
    mask = np.abs(wavelengths - line_center) <= window_width / 2.0
    if np.sum(mask) < 10:
        return 0.0
    ...
    # El continuo está normalizado a 1.0
    min_idx  = int(np.argmin(flux_win))
    min_flux = float(flux_win[min_idx])

    depth = 1.0 - min_flux
    if depth < 0.03:  # Línea demasiado débil para medir FWHM
        return 0.0

    # Nivel a mitad de la profundidad de absorción
    half_max_flux = min_flux + depth / 2.0

    # Cruce izquierdo
    left_cross = np.where(flux_win[:min_idx] > half_max_flux)[0]
    if len(left_cross) == 0:
        return 0.0
    left_boundary = float(wave_win[left_cross[-1]])

    # Cruce derecho
    right_cross = np.where(flux_win[min_idx:] > half_max_flux)[0]
    if len(right_cross) == 0:
        return 0.0
    right_boundary = float(wave_win[min_idx + right_cross[0]])

    fwhm = right_boundary - left_boundary
    if fwhm <= 0.5 or fwhm >= window_width:
        return 0.0
...
def measure_diagnostic_lines(wavelengths, flux_normalized, use_fwhm_adaptation=True,
    ...
    for line_name, line_wave in SPECTRAL_LINES.items():
        if wave_min <= line_wave <= wave_max:
            ...
            # ── FWHM de Balmer (ensanchamiento Stark → indicador de log g) ──
            # Solo para H_beta, H_gamma, H_delta (Hα menos útil en clasificación
            # ya que su región suele estar contaminada por emisión nebular/cromosférica)
            if line_name in ('H_beta', 'H_gamma', 'H_delta'):
                balmer_fwhm = estimate_balmer_fwhm(
                    wavelengths, flux_normalized, line_wave,
                    window_width=max(40.0, win * 2.0)
                )
                measurements[line_name]['fwhm'] = balmer_fwhm
...
def plot_spectrum_corrected(wavelengths, flux_normalized, measurements,
    ...
    fig = plt.figure(figsize=(28, 8 * n_rows))  # Figura grande para evitar solapamiento
    gs = fig.add_gridspec(n_rows, 1, height_ratios=[3.5, bar_panel_height] + [2.5] * n_zoom_panels, hspace=0.45)
    ...
    # Marcar líneas con EW significativo — etiquetas escalonadas con annotate para evitar solapamiento
    lines_to_label = []
    for line_name, data in measurements.items():
        if data['ew'] > 0.05:
            ...
            lines_to_label.append((data['wavelength'], line_name, color))

    lines_to_label.sort(key=lambda x: x[0])
    # 6 niveles de altura para mayor separación
    y_levels = [1.05, 1.18, 1.31, 1.44, 1.57, 1.70]
    min_gap_ang = 30  # Å mínimos entre líneas para reiniciar nivel
    last_wave = -9999
    last_level = -1
    for wave, line_name, color in lines_to_label:
        if wave - last_wave < min_gap_ang:
            level = (last_level + 1) % len(y_levels)
        else:
            level = 0
        y_tip = 1.02  # punta de la flecha toca justo encima del continuo
        y_label = y_levels[level]
        ax1.annotate(
            line_name.replace('_', ' '),
            xy=(wave, y_tip),
            xytext=(wave, y_label),
            ...
            bbox=dict(boxstyle='round,pad=0.15', facecolor='white', edgecolor=color, alpha=0.75, lw=0.6)
        )
        last_wave = wave
        last_level = level
    ...
    for i, region in enumerate(diagnostic_regions):
        ...
        # Marcar líneas en esta región — etiquetas escalonadas para evitar solapamiento
        zoom_lines = []
        for line_info in region['lines']:
            ...
            if line_name in measurements:
                ...
                zoom_lines.append((line_wave, line_name, ew, depth, color))

        # Ordenar por longitud de onda y escalonar etiquetas
        zoom_lines.sort(key=lambda x: x[0])
        region_span = region['wave_max'] - region['wave_min']
        min_gap_zoom = region_span * 0.08  # 8% del ancho de la región
        y_zoom_levels = [1.12, 1.22, 1.32, 1.42]
        last_z_wave = -9999
        last_z_level = -1
        for line_wave, line_name, ew, depth, color in zoom_lines:
            if line_wave - last_z_wave < min_gap_zoom:
                z_level = (last_z_level + 1) % len(y_zoom_levels)
            else:
                z_level = 0
            label_text = f'{line_name.replace("_", " ")}\nEW={ew:.2f}Å'
            if depth > 0.05:
                label_text += f'\nProf={depth:.2f}'
            ax_zoom.text(line_wave, y_zoom_levels[z_level], label_text,
                       ha='center', va='bottom', fontsize=9,
                       color=color, fontweight='bold',
                       bbox=dict(boxstyle='round,pad=0.3', facecolor='white',
                               edgecolor=color, alpha=0.7))
            last_z_wave = line_wave
